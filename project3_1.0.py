...
class networks: 
    def __init__(self,tasknum,xtrain,ytrain,xval,yval,categories):
        ...
        self.yvaltruth=self.yvaltruth[:,self.tasknum+1]
        encode=labelvalues()
        ...
        self.yvaltruth=encode.encode(self.yvaltruth,categories[self.tasknum])
        #self.yvaltruth,b=pd.factorize(self.yvaltruth)
        #self.yvaltruth=keras.utils.to_categorical(self.yvaltruth)
        self.yvaltruth=self.yvaltruth.astype('int64')

    def fcnn(self,lr=0.1,lrfactor=0.2,mom=0.1,epochs=2):#lr=learning rate, lrd=learning rate decay, mom=momentum,tasknum=what to classify
        self.lr=lr
        ...
        cm_plot_labels=self.categories[self.tasknum]
        ...
        cmdisp=sklearn.metrics.ConfusionMatrixDisplay(confusion_matrix=confmatrix,display_labels=cm_plot_labels)
        cmdisp.plot()
        plt.show()
...
# Task 1, FCNN
NETWORKS=networks(tasknum=0,xtrain=train_images,ytrain=train_labels,xval=val_images,yval=val_labels,categories=categories)
FCNN=NETWORKS.fcnn()
## Task 2, CNN
CNN=NETWORKS.cnn()
## Task 3, homebrew CNN
